...
import { GROUND_TOKEN_KEY } from "@/domains/attendance/ground-token";
import { formatTime } from "@/lib/datetime";

type Scan = Awaited<ReturnType<typeof groundScanFn>>;

/** Shared live check-in console for seminar day and exam day mode. */
export function GroundConsole({ loginPath }: { loginPath: string }) {
  ...
  function clearAccess() {
    ...
    queryClient.removeQueries({ queryKey: ["ground"] });
    navigate({ to: loginPath });
  }

  async function signOut() {
    if (token) await groundSignOutFn({ data: { token } }).catch(() => undefined);
    clearAccess();
  }
  ...
  if (!token) {
    return (
      <div className="mx-auto max-w-md space-y-4 px-6 py-16 text-center">
        <p className="text-sm text-muted-foreground">Sign in to a session to open the console.</p>
        <Button onClick={() => navigate({ to: loginPath })}>Back to sign in</Button>
      </div>
    );
  }
  ...
  if (board.error) {
    return (
      <div className="mx-auto max-w-md space-y-4 px-6 py-16 text-center">
        ...
        <Button onClick={clearAccess}>Back to sign in</Button>
      </div>
    );
  }

  const data = board.data!;
  const isExam = data.purpose === "EXAM";
    ...
    <div className="min-h-screen bg-surface-low">
        ...
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-sidebar-foreground/70">
              {data.session.venue} · {formatTime(data.session.startsAt)} · {data.staffName}
            </p>
          </div>
          <button
            type="button"
      ...
      <main className="mx-auto max-w-3xl space-y-6 px-6 py-8">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <StatCard
            label={isExam ? "Absent" : "Seats left"}
            value={
              isExam
                ? Math.max(data.stats.registered - data.stats.present, 0)
                : data.stats.seatsLeft
            }
          />
        </div>
        ...
        <section className="surface-card space-y-4 rounded-2xl p-6">
          <div className="flex items-center justify-between gap-4">
            <h2 className="text-lg font-semibold text-foreground">Scan a student pass</h2>
          ...
          {isExam && (
            <p className="text-xs text-muted-foreground">
              Exams require a hall ticket — walk-ins cannot be recorded.
            </p>
          )}
            ...
            {data.attendance.map((a) => (
              <li key={a.id} className="flex items-center justify-between px-6 py-4 text-sm">
                ...
                <span className="text-xs text-muted-foreground">
                  {a.is_walk_in ? "Walk-in" : "Registered"} · {formatTime(a.scanned_at)}
                </span>
              </li>
            ))}
          </ul>
