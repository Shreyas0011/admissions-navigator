...
type Purpose = "SEMINAR" | "EXAM";

/** Shared name + password sign-in used by both seminar day and exam day mode. */
export function GroundLogin({ purpose, consolePath }: { purpose: Purpose; consolePath: string }) {
  const navigate = useNavigate();
  const [selected, setSelected] = useState<string | null>(null);
  const [staffName, setStaffName] = useState("");
  const [password, setPassword] = useState("");

  const label = purpose === "EXAM" ? "Exam day mode" : "Seminar day mode";

  const { data, isLoading } = useQuery({
    queryKey: ["ground", "today", purpose],
    queryFn: () => listTodaySessionsFn({ data: { purpose } }),
  });

  const login = useMutation({
    mutationFn: () =>
      groundLoginFn({ data: { sessionId: selected!, staffName, password, purpose } }),
    onSuccess: (result) => {
      localStorage.setItem(GROUND_TOKEN_KEY, result.token);
      navigate({ to: consolePath });
    },
      ...
      <header className="bg-sidebar px-6 py-6">
        <div className="mx-auto flex max-w-3xl items-center gap-3">
          ...
          <span>
            ...
            <span className="block text-xs text-sidebar-foreground/70">{label}</span>
          </span>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-3xl space-y-6 px-6 py-10">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-foreground">
            Today&apos;s {purpose === "EXAM" ? "exams" : "sessions"}
          </h1>
        ...
        {isLoading ? (
          ...
          <p className="surface-card rounded-2xl p-8 text-center text-sm text-muted-foreground">
            No published {purpose === "EXAM" ? "exam" : "seminar"} session is scheduled for today.
          </p>
        ) : (
        ...
        {selected && (
          ...
          >
            ...
            <div className="space-y-2">
              <Label htmlFor="groundPassword">Session password</Label>
              <Input
                id="groundPassword"
                type="password"
                required
